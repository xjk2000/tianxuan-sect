...
3. 拆分任务并标注并行关系
   - 任务 1: 用户服务层（阵堂）
   - 任务 2: 用户控制器（阵堂）
   - 任务 3: 用户界面（器堂）【可与任务 1-2 并行】
   - 任务 4: 集成测试（执法堂）【依赖任务 1-3 全部完成】
...
1. 分配给各堂长老
   - 任务 1-2: @阵堂长老
   - 任务 3: @器堂长老（可与阵堂并行）
   - 任务 4: @执法堂长老（等待阵堂+器堂完成）

2. 并行调度策略
   - 阵堂和器堂基于契约可独立推进 → 默认并行调度
   - 器堂依赖阵堂真实实现 → 串行调度
   - 执法堂等所有开发任务完成后再启动
...
### 阶段 2: 开发（并行）
- 并行组 A: @阵堂长老 - 后端开发（任务 1-2）
- 并行组 B: @器堂长老 - 前端开发（任务 3）
- 共享边界: API 契约（天玄宗/契约中心/API契约/api-spec.yaml）
...
**并行可行性判断**：
- ✅ 前后端只共享 API 契约，各自可独立推进 → 并行
- ✅ 器堂可基于契约 + Mock 数据开发 → 并行
- ❌ 前端必须等后端真实接口联通才能开始 → 串行
